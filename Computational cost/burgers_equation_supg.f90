...
module supg_burgers_mod
  ...
  ! Discretization parameters - CHANGED: now variables instead of parameters
  integer :: n_elements = 128   ! Default number of elements
  integer :: n_nodes            ! Number of nodes (will be n_elements+1)
  real(wp) :: dx                ! Element size (will be calculated)
...
program burgers_equation_supg
  ...
  ! Check for command line arguments to set n_elements
  num_args = command_argument_count()
  if (num_args >= 1) then
    call get_command_argument(1, arg_val)
    read(arg_val, *) n_elements
  end if
  
  ! Calculate dependent values based on n_elements
  n_nodes = n_elements + 1
  dx = (x_max - x_min) / n_elements
