...
! gfortran -cpp -DPRECISION_FP16 heat_equation_fem_ks.f90 -o heat_fem_ks_fp16
! gfortran -cpp -DPRECISION_FP32 heat_equation_fem_ks.f90 -o heat_fem_ks_fp32
! gfortran -cpp heat_equation_fem_ks.f90 -o heat_fem_ks_fp64
! gfortran -cpp -DPRECISION_FP128 heat_equation_fem_ks.f90 -o heat_fem_ks_fp128

! Compile with ROOFLINE support:
! gfortran -cpp -DROOFLINE -DPRECISION_FP16 roofline_counter.o heat_equation_fem_ks.f90 -o heat_roof_ks_fp16
! gfortran -cpp -DROOFLINE -DPRECISION_FP32 roofline_counter.o heat_equation_fem_ks.f90 -o heat_roof_ks_fp32
! gfortran -cpp -DROOFLINE roofline_counter.o heat_equation_fem_ks.f90 -o heat_roof_ks_fp64
! gfortran -cpp -DROOFLINE -DPRECISION_FP128 roofline_counter.o heat_equation_fem_ks.f90 -o heat_roof_ks_fp128
...
contains
  ...
  subroutine solve_system()
    ...
    ! Simple Gaussian elimination with Kahan summation for more precise arithmetic
    do i = 1, n_nodes-1
  ...
  ! Calculate errors relative to exact solution using Kahan summation
  subroutine calculate_errors(end_time)
    ...
    ! Calculate errors with Kahan summation
    error_l1 = 0.0_wp
    error_l2 = 0.0_wp
    error_linf = 0.0_wp
    ...
    error_l1 = kahan_sum(diff_array) / n_nodes
    error_l2 = sqrt(kahan_sum(diff_squared) / n_nodes)
  ...
  ! Output solution to CSV with proper time tracking
...
program heat_equation_fem
  ...
  use fem_heat_mod
  use kahan_mod, only: kahan_add_scalar  ! Add kahan module
#ifdef ROOFLINE
  ...
  ! Time stepping loop with mass monitoring and Kahan summation
  do t = 1, n_time_steps
    ! Update time with Kahan compensation
    call kahan_add_scalar(current_time, dt, time_c)
    ...
    if (mod(t, max(1, n_time_steps/100)) == 0 .or. t == n_time_steps) then
      ...
      write(unit_mass,'(i0,1x,es16.8,1x,es16.8,1x,es16.8)') t, current_time, M_now
