...
! Compile with:
! gfortran -cpp -DPRECISION_FP16 advection_equation_fem_ks.f90 -o advection_fem_ks_fp16
! gfortran -cpp -DPRECISION_FP32 advection_equation_fem_ks.f90 -o advection_fem_ks_fp32
! gfortran -cpp advection_equation_fem_ks.f90 -o advection_fem_ks_fp64
! gfortran -cpp -DPRECISION_FP128 advection_equation_fem_ks.f90 -o advection_fem_ks_fp128

! Compile with ROOFLINE support:
! gfortran -cpp -DROOFLINE -DPRECISION_FP16 roofline_counter.o advection_equation_fem_ks.f90 -o advection_roof_ks_fp16
! gfortran -cpp -DROOFLINE -DPRECISION_FP32 roofline_counter.o advection_equation_fem_ks.f90 -o advection_roof_ks_fp32
! gfortran -cpp -DROOFLINE roofline_counter.o advection_equation_fem_ks.f90 -o advection_roof_ks_fp64
! gfortran -cpp -DROOFLINE -DPRECISION_FP128 roofline_counter.o advection_equation_fem_ks.f90 -o advection_roof_ks_fp128

! Define precision module first to avoid circular dependencies
...
module kahan_mod
  use precision_mod, only: wp         ! Use the existing wp definition
  implicit none
contains
!---------------------------------------------------------------
...
contains
  ...
  function entropy_energy_trap(u, dx) result(e)
    ...
    if (n < 2) then
      ...
      e = 0.5_wp*dx * ( 0.5_wp*(u(1)**2 + u(n)**2) + kahan_sum(u(2:n-1)**2) )  ! Use kahan_sum
    end if
...
module fem_advection_mod
  use precision_mod
  use kahan_mod, only: kahan_axpy, kahan_add_scalar
  use entropy_mod, only: entropy_energy, entropy_energy_trap  ! Added import for entropy functions
#ifdef ROOFLINE
  ...
  ! Kahan compensation arrays
  real(wp), allocatable :: kahan_c(:)      ! Compensation array for solution update
  ...
  real(wp) :: current_time                ! Current simulation time
...
contains
  ...
  ! Solve one time step using explicit Euler method with Kahan summation
  subroutine solve_time_step()
    integer :: i
    real(wp), allocatable :: k1(:), k2(:), k3(:), k4(:), temp(:)
    allocate(k1(n_nodes), k2(n_nodes), k3(n_nodes), k4(n_nodes), temp(n_nodes))
    ...
    ! Calculate intermediate steps with standard arithmetic (these are temporary)
    temp = u + 0.5_wp*dt*k1
    k2 = -( matmul(K, temp) ) / mlump
#ifdef ROOFLINE
    ...
    temp = u + dt*k3
    k4 = -( matmul(K, temp) ) / mlump
#ifdef ROOFLINE
    call add_flops(2 * n_nodes + n_nodes * n_nodes + n_nodes) ! u + k3*dt, matmul, division
    call add_bytes(4 * n_nodes * wp) ! read K, u, k3, write k4
#endif

    ! Instead of standard update, use Kahan summation
    temp = (k1 + 2.0_wp*k2 + 2.0_wp*k3 + k4)/6.0_wp  ! Calculate weighted sum
    call kahan_axpy(u, dt, temp, kahan_c)             ! Apply u = u + dt*temp with compensation
#ifdef ROOFLINE
    call add_flops(5 * n_nodes) ! weighted sum of k terms
    call add_bytes(5 * n_nodes * wp) ! read k1,k2,k3,k4,u, write u
#endif

    u(n_nodes) = u(1)  ! Enforce periodicity
    
    deallocate(k1, k2, k3, k4, temp)
  end subroutine solve_time_step
  
  ! Run the simulation with explicit time stepping
  subroutine time_stepping(unit_ent)
    integer, intent(in) :: unit_ent  ! File unit for entropy output
    integer :: t
    real(wp) :: E  ! For entropy calculation
    ...
    do t = 1, n_time_steps
      ...
      if (mod(t, max(1, n_time_steps/100)) == 0 .or. t == n_time_steps) then
        ...
        write(unit_ent, '(I0,1X,G0,1X,G0)') t, current_time, E
        
        ! Print entropy to screen
        print '(a,i6,a,es10.3)', ' Entropy at step ', t, ': ', E
      end if
    end do
  ...
  subroutine calculate_errors(end_time)
    ...
    write(12, '(a,es16.8)') 'L1 Error: ', error_l1
    write(12, '(a,es16.8)') 'L2 Error: ', error_l2
    write(12, '(a,es16.8)') 'Linf Error: ', error_linf
    close(12)
  ...
  subroutine output_csv(step, time)
    ...
    ! Write header
    write(10, '(a,f12.6)') 'x,u,time=', time
    
    ! Write data
    do i = 1, n_nodes
      write(10, '(f12.6,a,es20.10)') x(i), ',', u(i)
    end do
...
program advection_equation_fem
  ...
  integer :: istat
  integer :: t, unit_ent
  real(wp) :: E0, E   ! Initial and running entropy
  character(len = 100) :: filename
#ifdef ROOFLINE
  ...
  write(unit_ent, '(I0,1X,G0,1X,G0)') 0, 0.0_wp, E0
  
  ! Run the time stepping with Kahan summation - now pass the file unit
  call time_stepping(unit_ent)
...
! ./advection_roof_ks_fp16
! ./advection_roof_ks_fp32
! ./advection_roof_ks_fp64
! ./advection_roof_ks_fp128